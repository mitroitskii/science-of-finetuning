...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--activation-cache-path", "-p", type=Path, default="~/data/activations"
    )
    parser.add_argument("--base-model", type=str, default="gemma-2-2b")
    parser.add_argument("--reasoning-model", type=str, default="gemma-2-2b-it")
    parser.add_argument("--crosscoder", type=str, default="L15")
    ...
    # Otherwise use the original PairedActivationCache approach
        ...
        # FIXME have to stick with validation only because on name mismatch for the split between the two datasets

        # print("Using PairedActivationCache")
        # crosscoder = load_dictionary_model(args.crosscoder).to(args.device)
        
        # for split in ["train", "validation"]:
        #     fw_dataset, reasoning_dataset = load_activation_dataset(
        ...
        #     )
        #     max_activations_reasoning = compute_max_activation(
        #         crosscoder,
        #         reasoning_dataset,
        #         args.device,
        #         args.batch_size,
        ...
        #         "max_activations_reasoning": max_activations_reasoning.tolist(),
        #     }
        ...
        fw_dataset, reasoning_dataset = load_activation_dataset(
            ...
            layer=args.layer,
            lmsys_name="OpenR1-Math-220k-formatted",
            fineweb_name="fineweb-1m-sample",
            lmsys_split="test-col-message_in_chat_template",
    ...
    path = Path("results") / f"max_acts_{args.crosscoder.split('/')[-2]}.json"
    ...
    if args.latent_activation_cache_path:
        ...
        # For original approach with separate FW and reasoning datasets
        if "train" in results:
            df["max_act_train"] = np.maximum(
                results["train"]["max_activations_fw"],
                results["train"]["max_activations_reasoning"],
            )
            df["max_act_reasoning_train"] = results["train"]["max_activations_reasoning"]
            df["max_act_fw_train"] = results["train"]["max_activations_fw"]

        if "validation" in results:
            df["max_act_val"] = np.maximum(
                results["validation"]["max_activations_fw"],
                results["validation"]["max_activations_reasoning"],
            )
            df["max_act_reasoning_val"] = results["validation"]["max_activations_reasoning"]
